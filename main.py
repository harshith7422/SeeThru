...
face_cascade = cv2.CascadeClassifier("/Users/harshithyvs/Desktop/seethru/models/haarcascade_frontalface_default.xml")

# Load the COCO class labels our YOLO model was trained on
LABELS = open("/Users/harshithyvs/Desktop/seethru/models/coco.names").read().strip().split("\n")
with open("/Users/harshithyvs/Desktop/seethru/models/coco.names", "r") as f:
    classes = [line.strip() for line in f.readlines()]
colors = np.random.uniform(0, 255, size=(len(classes), 3))

# Load our YOLO object detector trained on the COCO dataset (80 classes)
print("[INFO] loading YOLO from disk...")
net = cv2.dnn.readNetFromDarknet("/Users/harshithyvs/Desktop/seethru/models/yolov3.cfg", "/Users/harshithyvs/Desktop/seethru/models/yolov3.weights")
...
def play_buzzer_sound():
    pygame.mixer.music.load("/Users/harshithyvs/Desktop/seethru/models/buzzer.mp3")
    pygame.mixer.music.play()

# Function to save terminal output to PDF
def save_terminal_output_to_pdf(output_file, command):
    # Execute the command and capture its output
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    output, _ = process.communicate()
    output = output.decode()

    # Create a PDF file and write the terminal output to it
    c = canvas.Canvas(output_file, pagesize=letter)
    c.drawString(100, 700, "Terminal Output:")
    text_lines = output.split('\n')
    y_coordinate = 680
    for line in text_lines:
        c.drawString(100, y_coordinate, line)
        y_coordinate -= 20  # Adjust vertical spacing
    c.save()
...
while True:
    ...
    if time.time() - last_detected_time > alert_threshold:
        ...
        play_buzzer_sound()
        save_terminal_output_to_pdf("terminal_output.pdf", "python your_script.py")  # Save terminal output to PDF
